...
function stableMemoryId(record) {
  const raw = [record.scope, record.subject, record.key, record.value, record.sessionId || "", record.epoch || ""].join("|");
  let hash = 0;
...
function createMemoryRecord(input) {
  ...
  if (seed.sessionId != null && String(seed.sessionId).trim()) record.sessionId = String(seed.sessionId).trim();
  if (seed.epoch != null && String(seed.epoch).trim()) record.epoch = String(seed.epoch).trim();
  if (seed.reason != null && String(seed.reason).trim()) record.reason = String(seed.reason).trim();
  if (seed.severity != null && String(seed.severity).trim()) record.severity = String(seed.severity).trim();
  if (seed.constraintType != null && String(seed.constraintType).trim()) record.constraintType = String(seed.constraintType).trim();
...
function validateMemoryRecord(record) {
  ...
  for (const optional of ["sessionId", "epoch", "reason", "severity", "constraintType"]) {
    if (record[optional] != null && (typeof record[optional] !== "string" || !record[optional].trim())) {
      throw new Error(`${optional} must be non-empty string when present`);
    }
  }
  return true;
}
