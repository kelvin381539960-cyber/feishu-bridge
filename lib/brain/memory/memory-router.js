...
function routeMemory(input) {
  ...
  const negativeTopK = Math.max(0, Number(seed.negativeTopK == null ? Math.min(3, negativeItems.length) : seed.negativeTopK));
  const negativeTrimmed = controller.trimMemoryItems(negativeItems, { maxItems: negativeTopK });
  ...
  return createMemoryPack({
    ...
    routing: {
      ...
      selectedCount: recordsSelected.length,
      ...(seed.routing || {}),
    },
  });
}
