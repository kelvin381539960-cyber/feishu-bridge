...
function isWorthPersisting(turn, opts) {
  ...
  if (seed.source === "explicit" && seed.value) return true;
...
function createMemoryStore(options) {
  ...
  function compact() {
    ...
    if (records.length > opts.maxRecords) {
      ...
      const rest = records.filter((record) => record.scope !== "negative")
        .sort((a, b) => Date.parse(b.updatedAt) - Date.parse(a.updatedAt) || b.confidence - a.confidence || a.id.localeCompare(b.id));
  ...
  function upsert(recordInput) {
    ...
    const index = records.findIndex((item) => item.id === record.id || (
      ...
      item.key === record.key &&
      String(item.sessionId || "") === String(record.sessionId || "") &&
      String(item.epoch || "") === String(record.epoch || "")
    ));
  ...
  async function persistMemoryTurn(turn) {
    ...
    return upsert({
      ...
      sessionId: seed.sessionId || context.sessionId,
      epoch: seed.epoch || context.epoch,
  ...
  async function queryMemory(query) {
    ...
    let result = records.filter((record) => {
      ...
      if (q.sessionId && record.sessionId && record.sessionId !== q.sessionId) return false;
      if (q.sessionId && !record.sessionId && q.strictSession === true) return false;
      if (q.epoch != null && record.epoch && String(record.epoch) !== String(q.epoch)) return false;
      if (q.epoch != null && !record.epoch && q.strictEpoch === true) return false;
