...
const MEMORY_PACK_SCHEMA_VERSION = "p5.memory-pack.v1";
...
function createMemoryPack(input) {
  ...
  const pack = {
    schemaVersion: seed.schemaVersion || MEMORY_PACK_SCHEMA_VERSION,
...
function validateMemoryPack(pack) {
  ...
  if (pack.schemaVersion !== MEMORY_PACK_SCHEMA_VERSION) throw new Error("unsupported memory pack schemaVersion");
  ...
  if (!pack.routing || typeof pack.routing !== "object") throw new Error("memory pack routing metadata must be object");
