...
function resolveMode(env) {
  ...
  const raw = trimStr(e.FEISHU_OUTPUT_LIMIT_MODE).toLowerCase();
  if (raw === "segment" || raw === "split") return "segment";
  if ((raw === "truncate" || raw === "clip") && truthyEnv(e.FEISHU_OUTPUT_LIMIT_TRUNCATE_ENABLED)) return "truncate";
  return "off";
}
...
class FeishuLimitPlugin extends OutputPlugin {
  ...
  process(ctx, result) {
    ...
    if (!max || replyBody.length <= max || mode === "off") {
