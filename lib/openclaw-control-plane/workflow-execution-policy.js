 ...
 * v2 adds Forecast Layer metadata without changing classification or workflow type.
 */

const { SPECIALIZED_WORKFLOWS } = require("../feishu-cursor/workflows/workflow-registry");
const { buildForecastPolicy } = require("./forecast-policy");
...
function withForecast(policy, planTask) {
  const p = policy || {};
  const forecast = buildForecastPolicy({
    workflow: p.workflow,
    taskType: p.taskType,
    taskSize: p.taskSize,
    planTask,
    multiAgentRequired: p.multiAgentRequired,
  });
  return {
    ...p,
    forecast,
    forecastGate: forecast.forecastGate,
    expectedOutcome: forecast.expectedOutcome,
    nextStepPrediction: forecast.nextStepPrediction,
    failureModes: forecast.failureModes,
    riskForecast: forecast.riskForecast,
    rollbackPlan: forecast.rollbackPlan,
    executeDecision: forecast.executeDecision,
    learningSignals: forecast.learningSignals,
  };
}
...
function resolveWorkflowExecutionPolicy(input) {
  ...
  if (taskType === "research") {
    return withForecast(resolveResearchPolicy({
      planTask,
      qaContext,
      promptStage,
    }), planTask);
  ...
  if (taskType === "prd" || taskType === "code" || taskType === "solution") {
    return withForecast(pendingMultiAgentPolicy(taskType), planTask);
  }
