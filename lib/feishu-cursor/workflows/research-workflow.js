...
function loadResearchWorkflowV2Runner() {
  return require("../../openclaw-control-plane/research-workflow-runner").runResearchWorkflowV2;
}

function loadSpecializedSoloRunner() {
  return require("../runtime/specialized-solo-runner").runSpecializedSoloWithTrace;
}
...
class ResearchWorkflowPlugin extends WorkflowPlugin {
  ...
  async run(ctx) {
    ...
    if (useV2) {
      const runResearchWorkflowV2 = loadResearchWorkflowV2Runner();
      return workflowResult({
        type: "override",
    ...
    if (ctx.classification && ctx.classification.role === "specialized") {
      const runSpecializedSoloWithTrace = loadSpecializedSoloRunner();
      return workflowResult({
        type: "override",
