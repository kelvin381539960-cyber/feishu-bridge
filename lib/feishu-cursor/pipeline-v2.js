...
const { createFeatureFlags } = require("../brain/runtime/feature-flags");

function nowMs() {
  return Date.now();
}

function emitTelemetry(telemetry, name, payload) {
  if (!telemetry || typeof telemetry.emit !== "function") return;
  try {
    telemetry.emit(name, payload || {});
  } catch (_) {
    // Telemetry is non-critical by design.
  }
}

function emitStageLatency(deps, stage, startedAt, extra) {
  emitTelemetry(deps && deps.telemetry, "stage_latency", {
    stage,
...
function safeLogger(deps, level, ...args) {
  const logger = (deps && deps.logger) || console;
  const fn = logger && typeof logger[level] === "function" ? logger[level] : logger && logger.log;
  if (typeof fn === "function") fn.apply(logger, args);
}

function fallbackPlan(task, userTask, normalizedContext) {
  const messageId = normalizedContext && normalizedContext.messageId;
  const chatId = normalizedContext && normalizedContext.chatId;
  return {
    classification: {
      taskType: "general",
      workflowKey: "general",
      role: "general",
      confidence: 0,
      reasons: ["planning_fallback"],
    },
    relayDecision: { mode: "none", reason: "planning_fallback" },
    safety: { ok: true },
    prompt: { task, userTask, stage: "execute" },
    runner: { runnerType: "legacy", backendMode: "openclaw", agentProfile: "full" },
    dispatch: {
      task,
      opts: { chatId, messageId, sessionId: chatId || "" },
      route: { reasonCodes: ["planning_fallback"] },
    },
  };
}
...
function workflowHintEnabled(featureFlags) {
  return !!(featureFlags && featureFlags.isEnabled && featureFlags.isEnabled("workflowHint"));
}
...
function clarifyControlFooterEnabled(featureFlags) {
  return !!(featureFlags && featureFlags.isEnabled && featureFlags.isEnabled("clarifyFooter"));
}
...
function createFeishuCursorPipelineV2(deps) {
  ...
  const featureFlags = d.featureFlags && typeof d.featureFlags.isEnabled === "function"
    ? d.featureFlags
    : createFeatureFlags({ runtimeConfig: d.runtimeConfig, overrides: d.featureFlags });

  async function runPipelineV2Legacy(data) {
    const tPipelineStart = Date.now();
    const rawCompatContext = normalizeCompatContext({ data, routing: d.routing, runtimeConfig: d.runtimeConfig, receivedAtMs: tPipelineStart });
    const msgId = rawCompatContext.messageId;
    ...
    if (featureFlags.isEnabled("observability")) emitStageLatency(d, "parse", tParseStart, { messageId: msgId || "" });
    ...
    if (!routeCheck.ok) {
      emitTelemetry(d.telemetry, "route_skipped", { reason: routeCheck.reason, chatId: extracted.chatId || "", messageId: extracted.messageId || "" });
      return;
    }
    ...
    const tPlanningProbeStart = nowMs();
    ...
    try {
      ...
      safeLogger(d, "error", "[brain-planning] prePlan failed; fallback to legacy-compatible plan", e && e.message);
      emitTelemetry(d.telemetry, "planning_failed", { stage: "prePlan", chatId: extracted.chatId || "", messageId: extracted.messageId || "", error: e && e.message });
      probePlanned = fallbackPlan(planTask, planUserTask, normalizedContext);
    }
    if (featureFlags.isEnabled("observability")) emitStageLatency(d, "planning_pre", tPlanningProbeStart, { chatId: extracted.chatId || "", messageId: extracted.messageId || "" });
    ...
    if (freshEval.shouldReset) {
      ...
      emitTelemetry(d.telemetry, "conversation_fresh_reset", { chatId: extracted.chatId, messageId: extracted.messageId || "", reason: freshEval.reason });
    }
    ...
    const tPlanningFinalStart = nowMs();
    let planned;
    try {
      planned = planner.finalPlan({
      normalizedContext,
      envelope,
      task: planTask,
      userTask: planUserTask,
      ...
      safeLogger(d, "error", "[brain-planning] finalPlan failed; fallback to legacy-compatible plan", e && e.message);
      emitTelemetry(d.telemetry, "planning_failed", { stage: "finalPlan", chatId: extracted.chatId || "", messageId: extracted.messageId || "", error: e && e.message });
      planned = fallbackPlan(planTask, planUserTask, normalizedContext);
    }
    if (featureFlags.isEnabled("observability")) emitStageLatency(d, "planning_final", tPlanningFinalStart, {
      chatId: extracted.chatId || "",
      messageId: extracted.messageId || "",
      workflowType: planned && planned.classification && (planned.classification.workflowKey || planned.classification.taskType) || "general",
    });
    let classification = planned.classification;
    const relayDecision = planned.relayDecision;
    emitTelemetry(d.telemetry, "classification", { chatId: extracted.chatId, messageId: extracted.messageId || "", taskType: classification.taskType, workflowKey: classification.workflowKey, confidence: classification.confidence, relayReason: relayDecision.reason, relayMode: relayDecision.mode, solutionMode: classification.solutionMode, memoryMode: memoryModePipeline });
    ...
    if (relayBody) {
      ...
      emitTelemetry(d.telemetry, "relay_short_circuit", { chatId: extracted.chatId, messageId: extracted.messageId || "", reason: relayDecision.reason, mode: relayDecision.mode });
      return;
    }
    ...
    const tMemoryStart = nowMs();
    let memory = null;
    if (featureFlags.isEnabled("memory")) {
      try {
        memory = await d.assembleMemoryContext({ chatId: extracted.chatId, sessionId: sessionIdForMemory, conversationEpochKey: rwKey, memoryMode: memoryModePipeline, task });
      } catch (e) {
        safeLogger(d, "error", "[feishu-memory] assemble failed; skipped", e && e.message);
        emitTelemetry(d.telemetry, "memory_failed", { stage: "assemble", chatId: extracted.chatId || "", messageId: extracted.messageId || "", error: e && e.message });
        memory = null;
      }
    } else {
      emitTelemetry(d.telemetry, "feature_flag_disabled", { flag: "memory", chatId: extracted.chatId || "", messageId: extracted.messageId || "" });
    }
    let memoryPack = memory && memory.memoryPack ? memory.memoryPack : null;
    if (featureFlags.isEnabled("observability")) emitStageLatency(d, "memory", tMemoryStart, {
      chatId: extracted.chatId || "",
      messageId: extracted.messageId || "",
      memoryEnabled: featureFlags.isEnabled("memory"),
      memoryTokenUsage: memoryPack && memoryPack.tokenUsage ? memoryPack.tokenUsage : (memory && memory.tokenUsage) || null,
    });
    ...
    if (memoryPack) {
      try {
        ...
        safeLogger(d, "error", "[feishu-memory] invalid memoryPack; skipped", e && e.message);
        emitTelemetry(d.telemetry, "memory_failed", { stage: "validate", chatId: extracted.chatId || "", messageId: extracted.messageId || "", error: e && e.message });
        memory = null;
        memoryPack = null;
    ...
    let controlPlanned;
    try {
      controlPlanned = planner.rebaseFinalPlan({
      normalizedContext,
      basePlan: planned,
      classification,
      ...
      safeLogger(d, "error", "[brain-planning] rebaseFinalPlan failed; fallback to legacy-compatible plan", e && e.message);
      emitTelemetry(d.telemetry, "planning_failed", { stage: "rebaseFinalPlan", chatId: extracted.chatId || "", messageId: extracted.messageId || "", error: e && e.message });
      controlPlanned = fallbackPlan(task, planUserTask, normalizedContext);
    }
    ...
    if (!gateResult.ok) {
      ...
      emitTelemetry(d.telemetry, "pipeline_gate_violation", { chatId: extracted.chatId, messageId: extracted.messageId || "", violations: gateResult.violations });
    ...
    if (executionPolicy.multiAgentRequired && !rwV2 && planClassification.taskType === "research" && prompt.stage === "execute") {
      emitTelemetry(d.telemetry, "research_v2_forced_by_policy", { chatId: extracted.chatId, messageId: extracted.messageId || "" });
    }
    ...
    emitTelemetry(d.telemetry, "ack_sent", { traceId: taskContext.traceId, chatId: extracted.chatId, messageId: extracted.messageId || "", ackMode: ack.mode, ackSent: ack.sent });
    ...
    const queued = await d.taskQueue.enqueue(async () => {
      ...
      const selectWorkflow = typeof d.selectWorkflowPlugin === "function" ? d.selectWorkflowPlugin : selectWorkflowPlugin;
      const wf = featureFlags.isEnabled("workflowPlugins") ? selectWorkflow({ classification: planClassification }) : null;
      if (!featureFlags.isEnabled("workflowPlugins")) {
        emitTelemetry(d.telemetry, "feature_flag_disabled", { flag: "workflowPlugins", chatId: extracted.chatId || "", messageId: extracted.messageId || "" });
      }
      if (wf) {
        let wfResult;
        const tWorkflowStart = nowMs();
        try {
          emitTelemetry(d.telemetry, "plugin_execution", { pluginType: "workflow", workflowType: planClassification.workflowKey || planClassification.taskType || "general", status: "start", chatId: extracted.chatId || "", messageId: extracted.messageId || "" });
          wfResult = await wf.run({
          classification: planClassification,
          prompt,
          executionPolicy,
          rwV2,
          ...
          emitTelemetry(d.telemetry, "plugin_execution", { pluginType: "workflow", workflowType: planClassification.workflowKey || planClassification.taskType || "general", status: "ok", chatId: extracted.chatId || "", messageId: extracted.messageId || "" });
        } catch (e) {
          safeLogger(d, "error", "[workflow-plugin] failed; fallback to original execution", e && e.message);
          emitTelemetry(d.telemetry, "plugin_execution", { pluginType: "workflow", workflowType: planClassification.workflowKey || planClassification.taskType || "general", status: "error", chatId: extracted.chatId || "", messageId: extracted.messageId || "", error: e && e.message });
          if (featureFlags.isEnabled("observability")) emitStageLatency(d, "workflow_plugin", tWorkflowStart, { chatId: extracted.chatId || "", messageId: extracted.messageId || "", fallback: true });
          return originalExecution();
        }
        if (featureFlags.isEnabled("observability")) emitStageLatency(d, "workflow_plugin", tWorkflowStart, { chatId: extracted.chatId || "", messageId: extracted.messageId || "", workflowType: planClassification.workflowKey || planClassification.taskType || "general" });
    ...
    if (!sv.ok && planClassification.role === "specialized") {
      emitTelemetry(d.telemetry, "specialized_trace_missing", { chatId: extracted.chatId, messageId: extracted.messageId || "", code: sv.code, reason: sv.reason });
    ...
    let formattedReply = d.formatCursorAdhocReply(r, { sourceMessageId: String(extracted.messageId || msgId || ""), onRequestIdMismatch: () => emitTelemetry(d.telemetry, "artifact_request_id_mismatch", { chatId: extracted.chatId, messageId: extracted.messageId || "" }) });
    if (planClassification.workflowKey === "research" && prompt.stage === "execute" && typeof r.code === "number" && r.code === 0) {
      ...
      if (stripped.length < formattedReply.length) emitTelemetry(d.telemetry, "reply_strip_process_narration", { chatId: extracted.chatId, messageId: extracted.messageId || "", removedChars: formattedReply.length - stripped.length });
      formattedReply = stripped;
    }
    if (prompt.stage === "clarify" && clarifyControlFooterEnabled(featureFlags)) {
    ...
    const outputResult = await runOutputPlugins({
      ...
      executionResult: r,
      metadata: {},
    }, featureFlags.isEnabled("outputPlugins") ? d.outputPlugins : []);
    if (!featureFlags.isEnabled("outputPlugins")) {
      emitTelemetry(d.telemetry, "feature_flag_disabled", { flag: "outputPlugins", chatId: extracted.chatId || "", messageId: extracted.messageId || "" });
    }
    if (featureFlags.isEnabled("observability")) emitStageLatency(d, "output_plugins", tOutputStart, { chatId: extracted.chatId || "", messageId: extracted.messageId || "", outputPluginsEnabled: featureFlags.isEnabled("outputPlugins") });
    ...
    if (classification.taskType === "research" && prompt.stage === "execute") {
      if (r.code !== 0) {
        ...
        emitTelemetry(d.telemetry, "research_execute_failed_snapshot", { chatId: extracted.chatId, messageId: extracted.messageId || "", code: r.code });
      }
      clearResearchWorkflowState(rwKey);
    }
    ...
    emitTelemetry(d.telemetry, "reply_sent", { traceId: taskContext.traceId, chatId: extracted.chatId, messageId: extracted.messageId || "", taskType: classification.taskType, replyStatus: "sent", exportKind: exportKind || null, longReplyDocExport: !!longReplyDocExport });
  }

  return async function runPipelineV2(data) {
    if (!featureFlags.isEnabled("newKernel")) {
      if (typeof d.legacyPipeline !== "function") {
        const err = new Error("FEISHU_BRAIN_NEW_KERNEL is off but deps.legacyPipeline is missing");
        err.code = "LEGACY_PIPELINE_MISSING";
        emitTelemetry(d.telemetry, "rollback_legacy_pipeline_missing", { reason: "feature_flag", flag: "newKernel" });
        throw err;
      }
      emitTelemetry(d.telemetry, "rollback_legacy_pipeline", { reason: "feature_flag", flag: "newKernel" });
      return d.legacyPipeline(data);
    }
