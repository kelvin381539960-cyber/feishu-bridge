...
function createFeishuCursorPipelineV2(deps) {
  ...
  async function runPipelineV2Legacy(data) {
    ...
    if (memoryInjected) {
      d.logger.log(
        "[feishu-memory] memoryPack injected(v2) chat=%s session=%s mode=%s tokens=%s",
        extracted.chatId,
        sessionIdForMemory || "(chatId)",
        memory.memoryMode || memoryModePipeline || "default",
        memoryPack.tokenEstimate
      );
    } else if (memory && memory.injected && !memoryPack) {
      d.logger.log(
        "[feishu-memory] ignored legacy text memory injection(v2) chat=%s session=%s mode=%s",
        extracted.chatId,
        sessionIdForMemory || "(chatId)",
        memory.memoryMode || memoryModePipeline || "default"
      );
  ...
  return async function runPipelineV2(data) {
    // P4: keep production path unchanged
