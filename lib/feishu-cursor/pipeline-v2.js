...
function isLikelyClarificationAnswer(text) {
  const s = String(text || "").trim();
  if (!s) return false;
  const numberedLines = s.match(/(?:^|\n)\s*(?:\d+[\.\)、]|[-*])\s+/g);
  const hasManyNumberedLines = !!(numberedLines && numberedLines.length >= 2);
  const hasAnswerLikeLead = /^(回答|补充|说明|范围|目标|场景|用户群体|竞品|维度)[:：\s]/i.test(s);
  return hasManyNumberedLines || hasAnswerLikeLead;
}
...
function buildWorkflowTraceHint(classification, prompt) {
  ...
  const parts = [];
  const wf = String(c.workflowKey || c.taskType || "general").trim() || "general";
  parts.push(wf);
  const stage = String(prompt && prompt.stage ? prompt.stage : "").trim();
...
function buildClarifyControlFooter() {
  // State machine: default to "continue next step" unless user explicitly says otherwise.
  return (
    "——\n" +
...
const { createBrainContext } = require("../brain/kernel/brain-context");
const { BRAIN_STAGE_REGISTRY } = require("../brain/kernel/stage-registry");
const { runBrainStages } = require("../brain/kernel/stage-runner");
...
function createFeishuCursorPipelineV2(deps) {
  ...
  async function runPipelineV2Legacy(data) {
    ...
    if (!data._feishuSkipDedupOnce) {
      if (await d.state.dedupConsume(msgId)) {
    ...
    if (extracted.messageType === "merge_forward" || extracted.reason === "empty_text") {
      const rawContent = data && data.message && data.message.content;
      d.logger.log(
        "[feishu-ws-cursor] raw content(v2):",
    ...
    if (extracted.skip) {
      if (extracted.reason === "unknown_type" && extracted.chatId) {
        await d.sendFeishuTextToChat(
          extracted.chatId,
          `暂不支持此消息类型(${extracted.message_type})。`
        );
      }
      return;
    }
    ...
    if (!routeCheck.ok) {
      if (d.telemetry) {
        d.telemetry.emit("route_skipped", {
          reason: routeCheck.reason,
          chatId: extracted.chatId || "",
          messageId: extracted.messageId || "",
        });
      }
      if (routeCheck.reason === "chat_not_allowed") {
        d.logger.log(
          "[feishu-ws-cursor] skip chat not in FEISHU_CURSOR_ALLOWED_CHAT_IDS chat=",
          extracted.chatId
        );
      }
      return;
    ...
    if (d.runtimeConfig.groupRequireAtBot && data.message) {
      ...
      if (d.shouldSkipGroupMessageWithoutAtBot(data.message, botOid)) {
        d.logger.log(
          "[feishu-ws-cursor] skip (群聊需 @ 机器人；或 bot open_id 不可用) chat=",
          extracted.chatId
        );
        return;
      }
    ...
    if (await d.state.consumeRecentOutboundReply(extracted.chatId, extracted.text || "")) {
      d.logger.log("[feishu-ws-cursor] skip echo reply for chat=", extracted.chatId);
      return;
    }
    ...
    if (extracted.messageType === "merge_forward" && !data._feishuMergeForwardSolo) {
      ...
      d.logger.log(
        "[feishu-ws-cursor] merge_forward debounced chat=%s waitMs=%s",
        extracted.chatId,
        d.state.mergeDebounceMs
      );
      return;
    }
    ...
    if (
      ...
      extracted.messageType === "post" ||
      extracted.messageType === "interactive"
    ) {
      ...
      if (pendingMerge) {
        ...
        if (exM && exM.media) {
          d.logger.log(
            "[feishu-ws-cursor] coalesced merge_forward + %s chat=%s",
            extracted.messageType,
            extracted.chatId
          );
          prefixedMediaText = (await processMedia(exM.media)) || "";
        }
      }
    }

    let task = extracted.text || "";
    if (prefixedMediaText) {
      task = task ? `${prefixedMediaText}\n\n${task}` : prefixedMediaText;
    }

    if (extracted.media) {
      d.logger.log(
        "[feishu-ws-cursor] processing media(v2):",
        extracted.messageType,
        JSON.stringify(extracted.media)
      );
    ...
    if (!routedTask.ok) {
      if (routedTask.reason === "empty_task") {
        await d.sendFeishuTextToChat(
          extracted.chatId,
          d.routing.direct
            ? "消息为空，请发送需要 Cursor 执行的文本任务。"
            : "请在触发前缀后输入任务描述（例如 /figma 在 Figma 里画登录页）。"
        );
      } else if (
        routedTask.reason === "prefix_miss" &&
        d.routing &&
        !d.routing.direct &&
        (d.runtimeConfig == null || d.runtimeConfig.prefixMissHintEnabled !== false)
      ) {
        const p = (d.routing.prefix && String(d.routing.prefix).trim()) || "/figma";
        await d.sendFeishuTextToChat(
          extracted.chatId,
          `前缀模式：消息请以「${p}」开头（例：${p} 任务说明）。任意文本触发请设 FEISHU_CURSOR_MODE=direct；关本提示设 FEISHU_PREFIX_MISS_HINT=0。`
        );
      }
      return;
    }
    ...
    if (parentContextInjected) {
      d.logger.log("[feishu-ws-cursor] parent context injected(v2), parent_id=", parentId);
    }
    ...
    const rwKey = researchWorkflowStateKey(
      extracted.chatId,
      d.runtimeConfig && d.runtimeConfig.openclawFeishuSessionNamespace
    );
    let researchRow = loadResearchWorkflowState(rwKey);
    const rcFirst =
      d.runtimeConfig == null || d.runtimeConfig.researchClarifyFirst !== false;
    const rwV2 = !!(d.runtimeConfig && d.runtimeConfig.researchWorkflowV2);
    ...
    const probeEnvelope = buildFeishuTaskEnvelope({
      data,
      extracted,
      routing: d.routing,
      runtimeMode: d.runtimeConfig && d.runtimeConfig.channelRuntimeMode,
      groupRequireAtBot: d.runtimeConfig && d.runtimeConfig.groupRequireAtBot,
      fullTaskPrefixes: d.runtimeConfig && d.runtimeConfig.fullTaskPrefixes,
      task: planTask,
      userTask: planUserTask,
      receivedAtMs: tPipelineStart,
    });
    const probePlanned = planOpenclawExecution({
      ...
      routing: d.routing,
      forceFull:
        /(feishu|larksuite)\./i.test(String(planTask)) || parentContextInjected,
    ...
    const probeSessionId =
      (probePlanned.dispatch &&
        probePlanned.dispatch.opts &&
        String(probePlanned.dispatch.opts.sessionId || "").trim()) ||
      "";
    const bumpFn =
      typeof d.bumpConversationEpoch === "function"
        ? d.bumpConversationEpoch
        : defaultMemoryEpoch.bumpConversationEpoch;
    const metaFn =
      typeof d.getLastTurnMetaForFresh === "function"
        ? d.getLastTurnMetaForFresh
        : defaultMemoryEpoch.getLastTurnMetaForFresh;
    const lastTurnMeta = metaFn({
      conversationKey: rwKey,
      sessionId: probeSessionId || extracted.chatId,
    });
    ...
    if (freshEval.shouldReset) {
      ...
      bumpFn({
        conversationKey: rwKey,
        newEpoch: extracted.messageId || msgId || "",
      });
      if (d.telemetry) {
        d.telemetry.emit("conversation_fresh_reset", {
          chatId: extracted.chatId,
          messageId: extracted.messageId || "",
          reason: freshEval.reason,
        });
      }
    }

    if (memoryModePipeline === "default") {
      const activeFollowup = activeWorkflowEvidence({
        researchRow,
        PHASE_CLARIFY_SENT,
        lastTurnMeta: lastTurnMeta || undefined,
        assistantLenThreshold: parseInt(process.env.FRESH_ACTIVE_ASSISTANT_MIN_LEN || "1200", 10),
      });
      if (detectFollowupWeak(userTaskForChain) && activeFollowup) {
        memoryModePipeline = "meta_followup";
      }
    }
    ...
    if (researchRow && researchRow.phase === PHASE_CLARIFY_SENT) {
      ...
      if (!explicitRestart && clarifyControl === "end") {
        ...
        researchRow = null;
        const endMsg =
          "好的，已结束本次调研。若要重新开始，请发送「新调研：<主题>」。";
        await d.state.rememberOutboundReply(extracted.chatId, endMsg);
        await d.sendFeishuChatReply(extracted.chatId, endMsg);
        return;
      }

      // Clarify stage is a state machine:
      // - Default: treat any next user message as clarification input and proceed to execute.
      // - Only if user explicitly says "继续澄清" do we stay in clarify.
      if (!explicitRestart && clarifyControl === "clarify") {
        classificationMerge = {
          ...
          reasons: ["research_continue_clarify_user"],
        };
        ...
        classificationMerge = {
          ...
          taskType: "research",
          stage: "execute",
          qaContext: clarifyControl === "next" ? "" : userTaskForChain.trim(),
          requiresTooling: true,
          requiresFullRunner: true,
          needsClarification: false,
          reasons: ["research_clarify_default_next_step"],
        };
        hadResearchContinuation = true;
        if (researchRow.originalUserTask) planUserTask = researchRow.originalUserTask;
        if (researchRow.originalTask) planTask = researchRow.originalTask;
    ...
    if (!classificationMerge && rcFirst === false) {
      const early2 = classifyOpenclawIntent({
      ...
      if (early2.taskType === "research") {
        classificationMerge = {
          stage: "execute",
          reasons: ["research_skip_clarify_env"],
        };
      }
    }

    const planned = planOpenclawExecution({
      envelope: buildFeishuTaskEnvelope({
        data,
        extracted,
        routing: d.routing,
        runtimeMode: d.runtimeConfig && d.runtimeConfig.channelRuntimeMode,
    ...
    if (d.telemetry) {
      d.telemetry.emit("classification", {
        chatId: extracted.chatId,
        messageId: extracted.messageId || "",
        taskType: classification.taskType,
        workflowKey: classification.workflowKey,
        confidence: classification.confidence,
        relayReason: relayDecision.reason,
        relayMode: relayDecision.mode,
        solutionMode: classification.solutionMode,
        memoryMode: memoryModePipeline,
      });
    }

    const relayBody = buildDeterministicRelayReply(
      userTaskForChain,
      data.message,
      botOpenId,
      d.isRelayLikeTask,
      d.runtimeConfig && d.runtimeConfig.relayPolicyMode
    );
    if (relayBody) {
      await d.state.rememberOutboundReply(extracted.chatId, relayBody);
      await d.sendFeishuChatReply(extracted.chatId, relayBody);
      if (d.telemetry) {
        d.telemetry.emit("relay_short_circuit", {
          chatId: extracted.chatId,
          messageId: extracted.messageId || "",
          reason: relayDecision.reason,
          mode: relayDecision.mode,
        });
      }
      return;
    }

    const sessionIdForMemory =
      planned.dispatch &&
      planned.dispatch.opts &&
      typeof planned.dispatch.opts.sessionId === "string" &&
      planned.dispatch.opts.sessionId.trim()
        ? planned.dispatch.opts.sessionId.trim()
        : "";

    const researchClarifySnapshot = {
      originalUserTask: planUserTask,
      originalTask: planTask,
    };

    const memory = await d.assembleMemoryContext({
      chatId: extracted.chatId,
      sessionId: sessionIdForMemory,
      conversationEpochKey: rwKey,
      memoryMode: memoryModePipeline,
      task,
    });
    const memoryInjected = !!(memory && memory.injected);
    if (memory && memory.injected) {
      task = memory.task;
      d.logger.log(
        "[feishu-memory] injected(v2) chat=%s session=%s mode=%s",
        extracted.chatId,
        sessionIdForMemory || "(chatId)",
        memory.memoryMode || memoryModePipeline || "default"
      );
    }
    const controlEnvelope = buildFeishuTaskEnvelope({
      data,
      extracted,
      routing: d.routing,
      runtimeMode: d.runtimeConfig && d.runtimeConfig.channelRuntimeMode,
    ...
    const controlPlanned = planOpenclawExecution({
      ...
      routing: d.routing,
      forceFull:
        /(feishu|larksuite)\./i.test(String(task)) || parentContextInjected,
      runtimeConfig: d.runtimeConfig,
      isRelayLikeTask: d.isRelayLikeTask,
      isReportLikeTask: d.isReportLikeTask,
      isResearchLikeTask: d.isResearchLikeTask,
    ...
    // Pipeline-gate-adapter：基于 controlPlanned.classification 做只读结构校验，
    // 不再做二次分类；违规会兜底回 general，并把违规码写入 dispatch.route.reasonCodes 及 telemetry。
    ...
    if (!gateResult.ok) {
      ...
      if (d.telemetry) {
        d.telemetry.emit("pipeline_gate_violation", {
          chatId: extracted.chatId,
          messageId: extracted.messageId || "",
          violations: gateResult.violations,
        });
      }
      if (d.logger && typeof d.logger.warn === "function") {
        d.logger.warn("[pipeline-gate] classification violation", {
          violations: gateResult.violations,
        });
      }
    }
    if (
      controlPlanned.dispatch &&
      controlPlanned.dispatch.route &&
      Array.isArray(controlPlanned.dispatch.route.reasonCodes)
    ) {
      const merged = [
        ...controlPlanned.dispatch.route.reasonCodes,
        ...gateResult.reasonCodes,
      ];
      controlPlanned.dispatch.route.reasonCodes = Array.from(new Set(merged));
    }

    const safety = controlPlanned.safety;
    const prompt = controlPlanned.prompt;
    const runner = controlPlanned.runner;
    const dispatch = controlPlanned.dispatch;
    if (
      memoryModePipeline === "ignore" &&
      dispatch &&
      dispatch.opts &&
      dispatch.opts.gatewayRequest &&
      typeof dispatch.opts.gatewayRequest === "object"
    ) {
      dispatch.opts.gatewayRequest = {
        ...dispatch.opts.gatewayRequest,
        sessionResetHint: true,
      };
    }
    const planClassification = controlPlanned.classification || {};
    const executionPolicy = resolveWorkflowExecutionPolicy({
      classification: planClassification,
      planTask: (dispatch && dispatch.task) || "",
      qaContext: planClassification.qaContext,
      promptStage: prompt.stage,
    });
    if (
      executionPolicy.multiAgentRequired &&
      !rwV2 &&
      planClassification.taskType === "research" &&
      prompt.stage === "execute" &&
      d.telemetry
    ) {
      d.telemetry.emit("research_v2_forced_by_policy", {
        chatId: extracted.chatId,
        messageId: extracted.messageId || "",
      });
    }
    ...
    const taskContext = buildTaskContext({
      ...
      prompt,
      meta: {
        traceSource: "pipeline_v2",
        channelRuntimeMode: d.runtimeConfig && d.runtimeConfig.channelRuntimeMode,
      },
    });
    if (d.telemetry) {
      d.telemetry.emit("policy_decision", {
        traceId: taskContext.traceId,
        chatId: extracted.chatId,
        messageId: extracted.messageId || "",
        taskType: planClassification.taskType,
        workflowKey: planClassification.workflowKey,
        taskSize: executionPolicy.taskSize,
        multiAgentRequired: executionPolicy.multiAgentRequired,
        decisionReason: executionPolicy.decisionReason,
        forcedRuntimeV2: executionPolicy.forcedRuntimeV2,
      });
    }
    ...
    const queued = await d.taskQueue.enqueue(async () => {
      const isResearchExecute =
        planClassification.taskType === "research" && prompt.stage === "execute";
      const useV2 =
        isResearchExecute && (rwV2 || executionPolicy.multiAgentRequired);
      const forcedV2 =
        isResearchExecute && executionPolicy.multiAgentRequired && !rwV2;
      if (useV2) {
      ...
      if (planClassification.role === "specialized") {
        return runSpecializedSoloWithTrace(d.runCursorAdhocPrompt, {
          dispatch,
          classification: planClassification,
          executionPolicy,
          promptStage: prompt.stage,
    ...
    let r = normalizeExecutionResult({
      ...rawQueued,
      executionPolicy,
      multiAgentRequired: executionPolicy.multiAgentRequired,
      runnerType: runner.runnerType,
      backendMode: runner.backendMode,
      queueMode: queued.metadata && queued.metadata.mode,
      queueWaitMs: queued.metadata && queued.metadata.queueWaitMs,
      queueDepth: queued.metadata && queued.metadata.queueDepth,
      agentProfile: runner.agentProfile,
      permissionMode: runner.permissionMode,
      cleanCwd: runner.cleanCwd,
      ackMode: ack.mode,
      degradeReason: runner.degradeReason,
      routeClass: dispatch.route && dispatch.route.routeClass,
      routeAgentId: dispatch.route && dispatch.route.agentId,
      routeReasonCodes: dispatch.route && dispatch.route.reasonCodes,
      sessionId: dispatch.opts && dispatch.opts.sessionId,
    });
    const effectiveMultiAgent =
      r.runtimeRunTrace && typeof r.runtimeRunTrace.multiAgentRequired === "boolean"
        ? r.runtimeRunTrace.multiAgentRequired
        : !!executionPolicy.multiAgentRequired;
    r = normalizeExecutionResult({
      ...r,
      multiAgentRequired: !!effectiveMultiAgent,
    });
    const sv = validateSpecializedRuntime({
      classification: planClassification,
      multiAgentRequired: !!effectiveMultiAgent,
      runtimeRunTrace: r.runtimeRunTrace,
    });
    if (!sv.ok && planClassification.role === "specialized") {
      if (d.telemetry) {
        d.telemetry.emit("specialized_trace_missing", {
          chatId: extracted.chatId,
          messageId: extracted.messageId || "",
          code: sv.code,
          reason: sv.reason,
      ...
      r = normalizeExecutionResult({
        ...r,
        code: 1,
        stdout: `交付被阻断（${sv.code}）：${sv.reason}`,
        stderr: String(sv.reason || ""),
        executionPolicy,
        multiAgentRequired: executionPolicy.multiAgentRequired,
      });
    }
    ...
    if (
      ...
      !hadResearchContinuation &&
      r.code === 0
    ) {
      markResearchClarifySent(rwKey, {
        originalUserTask: researchClarifySnapshot.originalUserTask,
        originalTask: researchClarifySnapshot.originalTask,
      });
    }

    if (d.telemetry) {
      d.telemetry.emit("runner_completed", {
        traceId: taskContext.traceId,
        chatId: extracted.chatId,
        messageId: extracted.messageId || "",
        taskType: classification.taskType,
        ...
        researchContinuation: hadResearchContinuation,
        researchWorkflowV2: rwV2,
        researchMeta: r.researchMeta || null,
        taskSize: executionPolicy.taskSize,
        multiAgentRequired: executionPolicy.multiAgentRequired,
        policyDecisionReason: executionPolicy.decisionReason,
      });
    }
    ...
    const timingRef =
      ...
      Number.isFinite(extracted.messageCreateTimeMs)
        ...
        : tPipelineStart;
    let formattedReply = d.formatCursorAdhocReply(r, {
      sourceMessageId: String(extracted.messageId || msgId || ""),
      onRequestIdMismatch: () => {
        if (d.telemetry) {
          d.telemetry.emit("artifact_request_id_mismatch", {
            chatId: extracted.chatId,
            messageId: extracted.messageId || "",
          });
        }
    ...
    if (
      planClassification.workflowKey === "research" &&
      prompt.stage === "execute" &&
      typeof r.code === "number" &&
      r.code === 0
    ) {
      const stripped = stripLeadingProcessNarration(formattedReply);
      if (stripped.length < formattedReply.length && d.telemetry) {
        d.telemetry.emit("reply_strip_process_narration", {
          chatId: extracted.chatId,
          messageId: extracted.messageId || "",
          removedChars: formattedReply.length - stripped.length,
        });
      }
    ...
    if (prompt.stage === "clarify" && clarifyControlFooterEnabled()) {
      ...
      if (footer && !String(formattedReply).includes("回复选项：")) {
        formattedReply = `${String(formattedReply || "").trim()}\n\n${footer}`;
      }
    }
    const body = d.appendFeishuTimingToReplyBody(
      formattedReply,
      Date.now(),
      timingRef,
      extracted
    );
    const finalBody = sanitizeRelayReplyBody(
      body,
      userTaskForChain,
      data.message,
      botOpenId,
    ...
    if ((process.env.FEISHU_DOC_EXPORT_DEBUG || "").trim() === "1") {
      d.logger.log(
        "[feishu-docx-export] pipeline",
        JSON.stringify({
          exportEnabled: exportEnabled(),
          exportKind,
          longReplyDocExport,
          researchHit:
            typeof d.isResearchLikeTask === "function"
              ? d.isResearchLikeTask(userTaskForChain)
              : null,
          reportHit:
            typeof d.isReportLikeTask === "function"
              ? d.isReportLikeTask(userTaskForChain)
              : null,
    ...
    try {
      const skipDocExport = prompt.stage === "clarify";
      ...
      if (!skipDocExport) {
        ...
        if (enriched && typeof enriched.replyBody === "string") {
          replyToSend = enriched.replyBody;
        }
        if (
          enriched &&
          typeof enriched.memoryReplyBody === "string" &&
          enriched.memoryReplyBody.trim()
        ) {
          taskContext.memoryReplyBody = enriched.memoryReplyBody;
        }
      }
    } catch (e) {
      d.logger.error("[feishu-docx-export] hook error", e && e.message);
    }
    ...
    if (classification.taskType === "research" && prompt.stage === "execute") {
      if (r.code !== 0) {
        const errText = String(r.stderr || r.stdout || (r.error && r.error.message) || "")
          ...
          .slice(0, 8000);
        const artBody =
          typeof taskContext.memoryReplyBody === "string" && taskContext.memoryReplyBody.trim()
            ? taskContext.memoryReplyBody
            : replyToSend;
        writeFailedResearchSnapshot(rwKey, {
          requestId: String(extracted.messageId || msgId || ""),
          originalUserTask: String(planUserTask || ""),
          workflow: "research",
          stage: "execute",
          error: errText || `exit:${r.code}`,
          lastKnownArtifactRef: firstHttpUrl(artBody),
        });
        if (d.telemetry) {
          d.telemetry.emit("research_execute_failed_snapshot", {
            chatId: extracted.chatId,
            messageId: extracted.messageId || "",
            code: r.code,
          });
    ...
    replyToSend = appendLlmUsageFooterToReply(replyToSend, r);

    const memoryPersistBody =
      typeof taskContext.memoryReplyBody === "string" && taskContext.memoryReplyBody.trim()
        ? taskContext.memoryReplyBody
        : replyToSend;

    await d.state.rememberOutboundReply(extracted.chatId, replyToSend);
    await d.sendFeishuChatReply(extracted.chatId, replyToSend);
    ...
    void d
      .persistMemoryTurn({
        ...
        conversationEpochKey: rwKey,
        userTask: userTaskForChain,
        replyBody: memoryPersistBody,
        cursorResult: r,
        taskContext,
        classification,
        workflowKey: String(
          (classification && classification.workflowKey) ||
            (classification && classification.taskType) ||
            ""
        ),
        artifactRef: firstHttpUrl(memoryPersistBody),
      })
      .then((ret) => {
        if (ret && ret.ok) {
          d.logger.log(
            "[feishu-memory] persisted(v2) chat=%s session=%s turns=%s",
            extracted.chatId,
            sessionIdForMemory || "(chatId)",
            ret.turnCount
          );
      ...
      .catch((e) => {
        d.logger.error("[feishu-memory] persist unexpected error(v2):", e && e.message);
      });

    await d.maybeChainAfterCursor({
      chatId: extracted.chatId,
      cursorResult: r,
      replyBody: replyToSend,
      userTaskBeforeInject: userTaskForChain,
      sendFeishuChatReply: d.sendFeishuChatReply,
      getBotSelfOpenId: d.getBotSelfOpenId,
    });
    if (d.telemetry) {
      d.telemetry.emit("reply_sent", {
        traceId: taskContext.traceId,
        chatId: extracted.chatId,
        messageId: extracted.messageId || "",
        taskType: classification.taskType,
  ...
  const stages = {
    classification: async (ctx) => {
      // full legacy pipeline executes here (behavior unchanged)
      await runPipelineV2Legacy(ctx.data);
    },
    memory: async () => {},
    policy: async () => {},
    ack: async () => {},
  ...
  return async function runPipelineV2(data) {
    ...
    return runBrainStages({
      registry: BRAIN_STAGE_REGISTRY,
      stages,
      ctx,
    });
  };
