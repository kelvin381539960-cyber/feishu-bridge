...
const { checkRoutingEligibility, resolveTaskAfterRouting } = require("./policies/routing-policy");
const { buildDeterministicRelayReply, sanitizeRelayReplyBody } = require("./policies/relay-policy");
...
function createFeishuCursorPipelineV2(deps) {
  ...
  async function runPipelineV2Legacy(data) {
    ...
    if (!routedTask.ok) {
      ...
      } else if (routedTask.reason === "prefix_miss" && d.routing && !d.routing.direct && (d.runtimeConfig == null || d.runtimeConfig.prefixMissHintEnabled !== false)) {
    ...
    const rwKey = researchWorkflowStateKey(extracted.chatId, d.runtimeConfig && d.runtimeConfig.openclawFeishuSessionNamespace);
    let researchRow = loadResearchWorkflowState(rwKey);
    const rcFirst = d.runtimeConfig == null || d.runtimeConfig.researchClarifyFirst !== false;
    ...
    const probeEnvelope = buildFeishuTaskEnvelope({ data, extracted, routing: d.routing, runtimeMode: d.runtimeConfig && d.runtimeConfig.channelRuntimeMode, groupRequireAtBot: d.runtimeConfig && d.runtimeConfig.groupRequireAtBot, fullTaskPrefixes: d.runtimeConfig && d.runtimeConfig.fullTaskPrefixes, task: planTask, userTask: planUserTask, receivedAtMs: tPipelineStart });
    ...
    if (!classificationMerge && rcFirst === false) {
      const early = classifyOpenclawIntent({ userTask: userTaskForChain, messageType: extracted.messageType, isRelayLikeTask: d.isRelayLikeTask, isReportLikeTask: d.isReportLikeTask, isResearchLikeTask: d.isResearchLikeTask });
      if (early.taskType === "research") classificationMerge = { stage: "execute", reasons: ["research_skip_clarify_env"] };
    }

    const envelope = buildFeishuTaskEnvelope({ data, extracted, routing: d.routing, runtimeMode: d.runtimeConfig && d.runtimeConfig.channelRuntimeMode, groupRequireAtBot: d.runtimeConfig && d.runtimeConfig.groupRequireAtBot, fullTaskPrefixes: d.runtimeConfig && d.runtimeConfig.fullTaskPrefixes, task: planTask, userTask: planUserTask, receivedAtMs: tPipelineStart });
    const planned = planOpenclawExecution({ envelope, task: planTask, userTask: planUserTask, classificationMerge, messageType: extracted.messageType, message: data.message, botOpenId, routing: d.routing, forceFull: /(feishu|larksuite)\./i.test(String(planTask)) || parentContextInjected, runtimeConfig: d.runtimeConfig, isRelayLikeTask: d.isRelayLikeTask, isReportLikeTask: d.isReportLikeTask, isResearchLikeTask: d.isResearchLikeTask, normalizeCursorTask: d.normalizeCursorTask, appendFeishuOpenIdMentionHint: d.appendFeishuOpenIdMentionHint, resolveCursorAgentProfile: d.resolveCursorAgentProfile, parentContextInjected });
    ...
    const relayBody = buildDeterministicRelayReply(userTaskForChain, data.message, botOpenId, d.isRelayLikeTask, d.runtimeConfig && d.runtimeConfig.relayPolicyMode);
    if (relayBody) {
    ...
    const controlEnvelope = buildFeishuTaskEnvelope({ data, extracted, routing: d.routing, runtimeMode: d.runtimeConfig && d.runtimeConfig.channelRuntimeMode, groupRequireAtBot: d.runtimeConfig && d.runtimeConfig.groupRequireAtBot, fullTaskPrefixes: d.runtimeConfig && d.runtimeConfig.fullTaskPrefixes, task, userTask: planUserTask, receivedAtMs: tPipelineStart, classification });
    const controlPlanned = planOpenclawExecution({ envelope: controlEnvelope, task, userTask: planUserTask, classificationMerge, messageType: extracted.messageType, message: data.message, botOpenId, routing: d.routing, forceFull: /(feishu|larksuite)\./i.test(String(task)) || parentContextInjected, runtimeConfig: d.runtimeConfig, isRelayLikeTask: d.isRelayLikeTask, isReportLikeTask: d.isReportLikeTask, isResearchLikeTask: d.isResearchLikeTask, normalizeCursorTask: d.normalizeCursorTask, appendFeishuOpenIdMentionHint: d.appendFeishuOpenIdMentionHint, resolveCursorAgentProfile: d.resolveCursorAgentProfile, parentContextInjected });
    ...
    if (controlPlanned.dispatch && controlPlanned.dispatch.route && Array.isArray(controlPlanned.dispatch.route.reasonCodes)) {
      controlPlanned.dispatch.route.reasonCodes = Array.from(new Set([ ...controlPlanned.dispatch.route.reasonCodes, ...gateResult.reasonCodes ]));
    }
    ...
    if (memoryPack && memoryInjected && dispatch) dispatch.opts = { ...(dispatch.opts || {}), memoryPack };
    if (memoryModePipeline === "ignore" && dispatch && dispatch.opts && dispatch.opts.gatewayRequest && typeof dispatch.opts.gatewayRequest === "object") {
      dispatch.opts.gatewayRequest = { ...dispatch.opts.gatewayRequest, sessionResetHint: true };
    }
    ...
    const taskContext = buildTaskContext({ chatId: extracted.chatId, messageId: extracted.messageId, messageType: extracted.messageType, rawTask: extracted.text || "", task, userTask: userTaskForChain, normalizedTask: prompt.task, mediaText: prefixedMediaText, parentContextInjected, mentionContextInjected, memoryInjected, sheetTaskDetected: planClassification.taskSubtype === "sheet_write", classification: planClassification, relayDecision, safety, memory: memory || null, prompt, meta: { traceSource: "pipeline_v2", channelRuntimeMode: d.runtimeConfig && d.runtimeConfig.channelRuntimeMode } });
    ...
    const queued = await d.taskQueue.enqueue(async () => {
      const originalExecution = async () => {
        const isResearchExecute = planClassification.taskType === "research" && prompt.stage === "execute";
        const useV2 = isResearchExecute && (rwV2 || executionPolicy.multiAgentRequired);
        const forcedV2 = isResearchExecute && executionPolicy.multiAgentRequired && !rwV2;
        if (useV2) {
          return runResearchWorkflowV2({ runOpenclawGatewayPrompt: d.runCursorAdhocPrompt, envelope: controlEnvelope, runtimeConfig: d.runtimeConfig, dispatch, classification, messageId: extracted.messageId, logger: d.logger, executionPolicy: { ...executionPolicy, forcedRuntimeV2: forcedV2 } });
        }
      ...
      if (wf) {
        const wfResult = await wf.run({ classification: planClassification, next: originalExecution });
    ...
    const resultPolicy = resolveOpenclawResultPolicy({ userTask: planUserTask, classification, structuredResult: r.structuredResult, resolveFeishuDocExportKind: d.resolveFeishuDocExportKind, isResearchLikeTask: d.isResearchLikeTask, isReportLikeTask: d.isReportLikeTask });
    const longMerge = mergeLongReplyDocExportKind({ exportKind: resultPolicy.exportKind, replyBody: finalBody, code: r.code, chatId: extracted.chatId });
    const exportKind = longMerge.exportKind;
    const longReplyDocExport = longMerge.longReplyForced;
    if ((process.env.FEISHU_DOC_EXPORT_DEBUG || "").trim() === "1") d.logger.log("[feishu-docx-export] pipeline", JSON.stringify({ exportEnabled: exportEnabled(), exportKind, longReplyDocExport, taskHead: String(userTaskForChain || "").slice(0, 160) }));
    try {
      if (prompt.stage !== "clarify") {
        const enriched = await docHook({ userTask: userTaskForChain, replyBody: finalBody, exportKind, logger: d.logger });
        if (enriched && typeof enriched.replyBody === "string") replyToSend = enriched.replyBody;
        if (enriched && typeof enriched.memoryReplyBody === "string" && enriched.memoryReplyBody.trim()) taskContext.memoryReplyBody = enriched.memoryReplyBody;
      }
    } catch (e) {
